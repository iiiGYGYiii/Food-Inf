...
async function searchFoodID(userResponse){
    ...
    try{
        const foodItems = [];
        // TRANSLATE USER INPUT TO ENGLISH WHICH IS THE LANGUAGE
        // THAT THE API REQUIRES
        const text = await translate(userResponse, {to:"en"});
        let url = API_URL + _.join(text.split(" ").map(word=> _.capitalize(word)), " ");
...
async function getFoodInf(foodItems){
    ...
    try{
        ...
        const foodItemsInf = await Promise.allSettled(foodIds.map(id => fetch(NUTRIENTS_URL,{
            ...fetchConfig,
            body: JSON.stringify({
                ingredients:[{
                ...foodJSON,
        ...
        const newArray = foodItemss.map(itm=>({
            ...foodItemsInf.find(item=>(item.value.ingredients[0].parsed[0].foodId == itm.id)&&item).value,
            ...itm
        })); 
        const sendFoodInf = [];
